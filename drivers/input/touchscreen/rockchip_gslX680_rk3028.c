...
static int gslX680_init(void)
{
	...
	this_ts->irq = of_get_named_gpio_flags(np, "touch-gpio", 0, (enum of_gpio_flags *)&irq_flags);
	//#if !defined(CONFIG_BOARD_TYPE_ZM1128CE) 
	#if   !defined(CANCEL_RESET)
	this_ts->rst = of_get_named_gpio_flags(np, "reset-gpio", 0, &rst_flags);
   ...
   // gpio_set_value(this_ts->rst, 1);
	...
	if(gpio_request(this_ts->rst,NULL)== 0){
	   printk(" *****I am gpio_request ok****\n");
	   gpio_direction_output(this_ts->rst, 1);
		//return -EIO;
	}else
	{
	    gpio_free(this_ts->rst);
		printk("this_ts->rst gpio_request error\n");
	}
	#endif
	msleep(20);
...
static void startup_chip(struct i2c_client *client)
{
  ...
  #ifdef GSLX680_COMPATIBLE
	if(chip_type==TP2680A_ID)
		gsl_DataInit(gsl_config_data_id_2680A);
	else
		gsl_DataInit(gsl_config_data_id_2680B);
...
static void report_data(struct gsl_ts *ts, u16 x, u16 y, u8 pressure, u8 id)
{
	...
	input_report_abs(ts->input, ABS_MT_POSITION_X, x);//1024
	input_report_abs(ts->input, ABS_MT_POSITION_Y, y);//600
	input_report_abs(ts->input, ABS_MT_WIDTH_MAJOR, 1);
#else
	input_report_abs(ts->input, ABS_MT_TRACKING_ID, id);
	input_report_abs(ts->input, ABS_MT_TOUCH_MAJOR, pressure);
	input_report_abs(ts->input, ABS_MT_POSITION_X, y);//1024
	input_report_abs(ts->input, ABS_MT_POSITION_Y,x);//600
	input_report_abs(ts->input, ABS_MT_WIDTH_MAJOR, 1);
